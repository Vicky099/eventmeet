...
# write here instead of parallel scan endpoints. Monthly range-partitioned on `scanned_at` — see
# lib/monthly_range_partitioning.rb for why (and for why this table has a composite primary key
# instead of the bare `id` every other table in this app uses).
class CreateScanEvents < ActiveRecord::Migration[8.0]
  def up
    MonthlyRangePartitioning.create_parent!(self, :scan_events, partition_column: :scanned_at) do |t|
