...
- [x] `PrintStation` model: `account_id`, `event_id`, `name`, printer mapping. → `app/models/print_station.rb`, `db/migrate/20260719000001_create_print_stations.rb`. `printer_name` is free text (blank = OS default printer) — round-tripping a live per-OS printer list back to the server was judged not worth the cross-platform fragility for what an admin can just type once. `pairing_code`/`pairing_code_expires_at` live directly on the station (`#generate_pairing_code!`, 10-minute TTL, `Participant#generate_unique_hex_id`'s own retry-until-unique shape) — only one pairing attempt is ever meaningful per station at a time.
- [x] `PrintAgent`: one-time pairing code generation from the admin console; once paired, issues a station-scoped JWT (`account_id`, `event_id`, `station_id`, short expiry) delivered over Action Cable. → `app/models/print_agent.rb`, `db/migrate/20260719000002_create_print_agents.rb`, `app/services/print_agent_token.rb` (`gem "jwt"`, signed with `Rails.application.secret_key_base` — no new credential to provision). 24h expiry, but **not** the real revocation mechanism — `PrintAgentToken.decode` re-checks `revoked_at` live against the DB on every use, exactly as the requirement's "immediately invalidating" demands. `PrintAgentController#pair`/`#badge` (`app/controllers/print_agent_controller.rb`) are the agent's two HTTP touchpoints, routed under the tenant subdomain constraint alongside `checkin` but with no Devise session (`skip_before_action :verify_authenticity_token` — found live: a bare POST from a non-browser client 500'd with `InvalidAuthenticityToken` before this was added; request specs didn't catch it since Rails' test env disables forgery protection by default).
- [x] `PrintJob`: queued per station, status tracking (pending/sent/failed/succeeded), reuses the pattern from baseline's bulk-print failure tracking. → `app/models/print_job.rb`, `db/migrate/20260719000004_create_print_jobs.rb`. Not partitioned like `ScanEvent` — bounded per-event volume, same shape as `Badge`/`Participant`.
- [x] Admin console: pairing-code generator UI, per-station printer assignment, per-event auto-print on/off toggle, connection-status indicator per paired station (online/offline via Cable presence). → `Admin::PrintStationsController`, `app/views/admin/print_stations/`. `PrintStation#online?` requires both `PrintAgent#connected` (toggled by `PrintJobsChannel#subscribed`/`#unsubscribed`) and a `last_seen_at` within `PrintStation::ONLINE_WINDOW` (45s, refreshed by the agent's own heartbeat action) — confirmed live that Cable's subscribed/unsubscribed toggle alone isn't enough (a test station correctly flipped to "Offline" once its `last_seen_at` staled out with no real Electron process sending heartbeats).
- [x] Server-side: on a qualifying scan (Phase 9), if auto-print is enabled for the event, render the badge (Phase 8) and push a `PrintJob` to the correct station's channel. → `PrintTriggerService` (`app/services/print_trigger_service.rb`), the one place printing actually happens — badge resolution, a 30s debounce mirroring `ScanService::DEBOUNCE_WINDOW`, and the dispatched-vs-fallback branch, called from `CheckinController#scan` when `event.auto_print_enabled?` (Event gained `auto_print_enabled`/`default_print_station_id`, `db/migrate/20260719000005_add_print_settings_to_events.rb`).
- [x] Revocation: an admin can revoke a station's pairing at any time, immediately invalidating its JWT. → `Admin::PrintStationsController#revoke` sets `PrintAgent#revoked_at` and calls `ActionCable.server.remote_connections.where(current_print_agent: agent).disconnect` — forces any live connection down immediately, not just on its next reconnect.
- [x] Document the agent-facing contract (channel name, JWT claims, job payload shape, ack/status-report format) in this file or a linked `doc/print-agent-protocol.md` so the Electron build (separate track) has a stable target. → `doc/print-agent-protocol.md`. **Found and fixed via that same documentation pass**: Action Cable's own `Channel::Base#perform_action` dispatches by the incoming message's `"action"` key as a literal method name (falling back to `#receive` only when no `"action"` key is present at all) — this protocol's own wire shape (`{"action": "job_update", ...}`) means `PrintJobsChannel` needed real public `#job_update`/`#heartbeat` actions, not a single `#receive` switching on `data["action"]` internally; a channel spec sending the intended payload and asserting the resulting `PrintJob` status caught this live (the status silently never updated) before it shipped.
- [x] **Not part of the original checklist, built alongside it (user request):** a real, minimal Electron agent (`print-agent/` at the repo root — a genuine separate package, not nested in `backend/`) that pairs, holds one Action Cable connection via `ws` (no `@rails/actioncable`, which isn't pinned anywhere in this app and expects a browser environment), fetches the badge PDF with its Bearer token, and calls `webContents.print({ silent: true, printBackground: true, deviceName })`. **Explicitly out of scope for this pass**, flagged rather than silently skipped: `electron-builder` packaging, code signing, `electron-updater` auto-update, and raw Zebra/Evolis driver support — real per-OS release-engineering work; runs via `npm start` (`electron .`) in dev today.
- [x] **Also not part of the original checklist (user request), participant/check-in print UX layered on top of the above:** a manual Print action on the Participants list (replacing "Resend invitation" in the table row only — Resend stays on the participant show page) and show page, dispatching via `PrintTriggerService` with the confirmed PDF-download fallback when no station is paired/online (`Admin::ParticipantsController#print`); a **Bulk Print** page (new `BulkPrintRun` model + `BulkPrintRunJob`, `app/models/bulk_print_run.rb`) — admin picks a station + batch limit, the run skips participants with an already-`succeeded` `PrintJob` (naturally resumable after a paper jam — no separate "resume" action needed), and the progress page shows a running completed-count plus the **last printed participant**, the literal "know where the printer stopped" ask; check-in gained a third Direction ("Print only" — reuses `ScanService`'s existing skip-`Attendance`-for-non-check-in/out-scan-types behavior unchanged) and an independent "Also print" toggle (hidden when Direction is Print only), requiring `checkin_controls_controller.js` to generalize from a single pill-group/chip-group (`pillGroupTarget` singular) to iterating `pillGroupTargets` (plural) since a second, independent pill-group now coexists with Direction.

### Definition of Done
- [x] Request/channel spec: pairing code redemption issues a correctly scoped JWT; an expired/revoked JWT is rejected on the next connection attempt. → `spec/requests/print_agent_spec.rb`, `spec/services/print_agent_token_spec.rb`.
- [x] Channel spec: a simulated agent connection (test WebSocket client, no real Electron needed) receives a `PrintJob` push when a qualifying scan occurs and auto-print is on. → `spec/channels/print_jobs_channel_spec.rb` (`ActionCable::Channel::TestCase`), `spec/requests/checkin_spec.rb`'s "auto_print_enabled on the event prints automatically with no operator toggle" case.
- [x] Request spec: auto-print off → no job pushed, badge still available via the Phase 8 on-demand download. → `spec/requests/checkin_spec.rb`.
- [x] Manual QA (once even a minimal agent stub exists): pair a station, scan a participant, confirm the job arrives at the agent process (doesn't need a real printer — logging "would print X" is sufficient to validate the contract). → Driven live against a real dev server (`acme.lvh.me:3000`, seeded tenant): generated a real pairing code from the Print Stations UI, redeemed it via `POST /print_agent/pair` (caught and fixed the CSRF bug above in the process), confirmed `/print_agent/print_jobs/:id/badge` streams a real PDF only for a valid, non-revoked Bearer token. Clicked the Participants list's Print button with no station paired → PDF opened inline as designed; with an online default station → "Print job sent to {station}" confirmation page, a real `PrintJob`/`ScanEvent` pair in the DB. Drove the check-in kiosk live: "Print only" produced no `Attendance` row; "Also print" on a Check-in produced both an `Attendance` row and a dispatched `PrintJob`. Ran a real Bulk Print batch (limit 3) against seeded participants, simulated two agent acks, and confirmed the progress page correctly read "2 / 3 printed — Last printed: {participant}". **Also found live** (pre-existing, unrelated to this phase's own diff — reproduced identically against the untouched Phase 8 `#badge` action): this long-lived dev DB's `scan_events` partitions hadn't been extended past their original migration-time window (`PartitionMaintenanceJob`'s own comment already flags this exact bootstrap gap) — ran it once manually to unblock verification; a real fix (wiring it onto `config/schedule.yml` like `EventSchedulerJob`/`InvoiceGenerationJob` already are) is a good follow-up but out of this phase's scope.
